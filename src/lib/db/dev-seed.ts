...
export async function seedDevelopmentDataAsync(db: DatabaseWriter): Promise<void> {
  ...
  await db.withExclusiveTransactionAsync(async (txn) => {
    for (const book of figmaBooks) {
      ...
      const currentPage =
        totalPages === null ? 0 : Math.round((totalPages * (book.progress ?? 0)) / 100);
      ...
      await txn.runAsync(
        ...
        [
          ...
          book.status === "finished" || book.status === "recent"
...
async function seedNotificationsAsync(txn: DatabaseWriter, now: string): Promise<void> {
  const notifications = [
    [
      "notification-share-streak",
      "share-streak",
      "12 day share streak",
      "Tap to share today's read and keep the streak alive.",
    ...
    [
      ...
      "The voice felt like someone finally told the truth about being poor.",
      "2026-05-14T08:00:00.000Z",
    ],
    [
      "notification-reading-window",
      "read-reminder",
      "Your 8:30 reading window",
      "Klara and the Sun • 127/303",
      "2026-05-14T08:30:00.000Z",
    ],
    [
      "notification-wrapped",
      "wrapped",
      "May wrapped is ready",
      "4 books • 1,021 pages • 12 day streak",
